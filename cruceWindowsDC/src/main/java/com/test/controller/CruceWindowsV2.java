...
public class CruceWindowsV2 {

    //private static final DateTimeFormatter formatter = DateTimeFormatter.ofPattern("EEEE d 'de' MMMM 'de' yyyy HH'H'mm'' z");
	public static final DateTimeFormatter formatter = DateTimeFormatter.ofPattern("EEEE, MMMM d, yyyy h:mm:ss a z");
    ...
    public static CiUcmdbWindows getRecentCiOld(CiUcmdbWindows firstCiUmcbd, CiUcmdbWindows secondCiUmcbd) {
        ...
        // Se separan las fechas que estabana grupadas, en un arreglos para los objetos firstCiUmcbd y secondCiUmcbd
        ...
        LocalDateTime maxFirstLastAccessTime = Stream.of(lastAccesTimeFirstCi)
                //.map(time -> LocalDateTime.parse(time, formatter))
        		 .map(time -> Libs.extractDateFromLastAccesstime(time, Libs.PATTERN_DATE))
    ...
    public List<MatchTowerUcmdb> towerInventaryMergeUcmdbInventary(String windowsInventaryPath, String ucmdbInventaryPath) throws Exception {
        ...
        if ((windowsInventary.size() == 0 || inventoryUCMDB.size() == 0)) {
            ...
            for (int i = 0; i < windowsInventary.size(); i++) {
                ...
                System.out.println(ciWindows.toString() + " |");
                ...
                if (inventoryUCMDBhm.containsKey(ciWindows.getCodHostname().toUpperCase())) {
                    ...
                    for (CiUcmdbWindows ciUcmdbWindows : inventoryUCMDB) {
                        ...
                        if (hasMatchIP && !hasMatch) {
                            ...
                            //SE TOTEA PORQUE NO TIENE _ UN REGISTRO 
                            String ciWindowsServCode = ciWindows.getCodHostname();
                            String ciWindowsDispLabel = ciWindows.getCodHostname();
                            if (!ciWindows.getCodHostname().contains("_")) {
                                ciWindowsServCode = Libs.extactServiceCode(ciWindows.getCodHostname());
                            } else {
                                ciWindowsServCode = ciWindows.getCodHostname().substring(0, ciWindows.getCodHostname().indexOf('_'));
                                ciWindowsDispLabel = ciWindows.getCodHostname().substring(ciWindows.getCodHostname().indexOf('_') + 1, ciWindows.getCodHostname().length());
                            }
                ...
                System.out.println(resultMatchTowerUcmdb.get(i).getStatusMatch());
            }

        }
        System.out.println("Termino el torre vs ucmdb");
    ...
    // Este método compara dos objetos de tipo CiServiceManager y devuelve el objeto con la fecha de modificación más reciente
    public CiServiceManager getRecentCiSM(CiServiceManager firstCiSM, CiServiceManager secondCiSM) {
        // Obtiene la fecha de modificación del primer objeto y segundo objeto CiServiceManager
        String firstCiSMDate = firstCiSM.getFechaModificacion();
        String secondCiSMDate = secondCiSM.getFechaModificacion();
        // Formateador de fecha utilizado para parsear las fechas de modificación
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yy HH:mm:ss");
        // Convierte la fecha de modificación del primer y del 2do objeto a un objeto LocalDateTime
        ...
        // Devuelve el objeto con la fecha de modificación más reciente
